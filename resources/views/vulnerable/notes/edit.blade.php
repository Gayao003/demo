<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Note (Vulnerable Implementation)
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4" role="alert">
                        ...
                        <p>This page is vulnerable to IDOR. You can edit notes from other users by changing the ID in the URL.</p>
                        <p class="mt-2">Current note belongs to user ID: {{ $note->user_id }}</p>
                        <p>Your user ID: {{ Auth::id() }}</p>
                    </div>

                    <form method="POST" action="{{ route('vulnerable.notes.update', $note->id) }}">
                        @csrf
                        @method('PUT')
